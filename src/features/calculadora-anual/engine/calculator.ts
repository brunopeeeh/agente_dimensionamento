...
import { buildTurnoverContext, resolveTurnoverForMonth, buildTurnoverFormula } from "./turnover";
...
const computeHCNominalStart = (state: SimulationState, activePastCohorts: HireCohort[]): number =>
  Math.max(
    0,
    state.legacyPleno + state.legacyRookie + activePastCohorts.reduce((acc, c) => acc + c.count, 0),
  );
...
const computeHCEffective = (
  ...
  const rookieRampFactor =
    index === 0
      ? safeRampFactors.month1
      : index === 1
        ? safeRampFactors.month2
        : safeRampFactors.month3;
...
const computeOpenTiming = (
  ...
  const openIn =
    openMonthIndex >= 0 && openMonthIndex < timeline.length
      ? timeline[openMonthIndex].label
      : "Antes do período";

  const targetImpactIndex = index + openOffset;
  const targetImpactLabel =
    targetImpactIndex < timeline.length ? timeline[targetImpactIndex].label : "Fora do período";
...
export const runPlannerProjection = (inputs: PlannerInputs): ProjectionResult => {
  ...
  for (let index = 0; index < timeline.length; index++) {
    ...
    const agentsNeededRaw = demand.volumeHuman / Math.max(1, baselineCapacityPerAgent);
  ...
  for (let index = 0; index < timeline.length; index++) {
    ...
    const adjustedMix = inputs.useN1N2Split
      ? computeAdjustedMix(
          inputs.mixN1Pct,
          inputs.mixN2Pct,
          inputs.promotionsCount,
          inputs.headcountCurrent,
          index,
          totalMonths,
        )
      : undefined;
    ...
    if (inputs.turnoverTiming === "start_of_month" && turnover > 0) {
      turnoverAppliedStart = applyTurnoverStartOfMonth(
        state,
        turnover,
        hcNominalStart,
        activePastCohorts,
    ...
    const hcFinal = Math.max(
      ...
      state.legacyPleno +
        state.legacyRookie +
        allActiveCohorts.reduce((acc, c) => acc + c.count, 0),
    );
